...
const OwnerDetail = ({navigation, route, userId}) => {
  ...
  const fetchUser = async () => {
    const apartmentSnapshot = await getDoc(
      doc(db, 'apartments', item.apartmentId),
    );
  ...
  const handleItemClick = async (item, apartmentId) => {
    navigation.navigate('SetApartment', {item, apartmentId});
  };
          ...
          <Menu
            ...
            <View>
              ...
              <Menu.Item
                onPress={() => {
                  handleMenuItemPress();
                  handleItemClick(item, item.apartmentId);
                }}
                    ...
                    [
                      ...
                      },
                      {
                        ...
                        onPress: async () => {
                          handleMenuItemPress();
                          try {
                            const apartmentId = item.apartmentId;
                            console.log(apartmentId); // Verify that the ID is correct
                            await deleteDoc(
                              doc(collection(db, 'apartments'), apartmentId),
                            ); // Use async/await to wait for the delete operation to complete
                            navigation.replace('Owners');
                          } catch (error) {
                            console.error('Error deleting document:', error); // Log any errors that occur
                          }
                        },
                      },
                    ],
